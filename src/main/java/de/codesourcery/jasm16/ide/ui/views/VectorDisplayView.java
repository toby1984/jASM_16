...
public class VectorDisplayView extends AbstractView
{
    public static final String VIEW_ID="vector-display-view";
    
    private volatile JPanel panel;

    private final IEmulator emulator;
    
    private volatile DefaultVectorDisplay screen = null;    
    ...
    public VectorDisplayView(IEmulationOptionsProvider optionsProvider, IEmulator emulator) 
    {
        if (emulator == null) {
    ...
    private void attach(final JPanel panel) 
    {
        ...
        UIUtils.invokeAndWait( new Runnable() {
            ...
            public void run()
            {
                screen = optionsProvider.getEmulationOptions().getVectorDisplay( emulator );
                screen.attach( panel );                
            }} );
    }    
        	...
        	public void paint(java.awt.Graphics g) 
             {
        		...
        		final BufferedImage original1 = screen.getImage();
        		if ( original1 == null ) {
        		    return;
        		}
